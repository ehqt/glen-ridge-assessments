    ...
    <article>
      <section className="prose">
        ...
        <p>
          ...
          by the other gives a <strong>ratio</strong>, and thousands of ratios
          together describe how the whole roll behaves. This is the same method
        ...
        <dl className="terms-list">
          ...
          <div>
            ...
            <dd>
              ...
              is the difference in median ratio between the cheapest fifth and
              the priciest fifth. A gap of −20% means the most expensive homes
      ...
      <section className="figures figures-town">
        <Figure
          label={`Assessment level ${latest.year}`}
          value={r3(latest.median)}
          sub={`state ratio ${r3(latest.eq)} · ${latest.n} sales`}
          accent
        />
        <Figure
          label="Uniformity (COD)"
          value={latest.cod == null ? "—" : latest.cod.toFixed(1)}
          sub={`${codVerdict} · ceiling ${codCeiling}`}
        />
        <Figure
          label="Vertical equity (PRB)"
          value={r3(latest.prb)}
          sub={
            latest.prb == null
              ? "—"
              : latest.prb < prbLo
              ? "regressive beyond IAAO range"
              : latest.prb > prbHi
              ? "progressive beyond IAAO range"
              : "within IAAO range"
          }
        />
        <Figure
          label={`Cheapest vs priciest ${activeYear}`}
          value={gap == null ? "—" : `${gap > 0 ? "+" : ""}${gap.toFixed(1)}%`}
          sub="ratio gap across value quintiles"
        />
      </section>
      ...
      <section className="chart-block">
        ...
        <p className="chart-note chart-intro">
          Each point is the median ratio of all sales that year: assessed value
          divided by what the property actually sold for. At 0.50 the roll is
          assessing at about half of market value.
        </p>
      ...
      <section className="chart-block">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={rows} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
            <XAxis dataKey="year" tickLine={false} axisLine={{ stroke: C.hairline }} tick={axis} />
            <YAxis tickLine={false} axisLine={false} width={44} tick={axis} />
            <Tooltip
              formatter={(v, name) =>
                Array.isArray(v)
