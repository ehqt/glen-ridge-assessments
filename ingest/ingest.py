...
def _levy_fields(r, ratables, rate):
    """Levy total and breakdown, tolerating gaps in the DCA tables.

    Precedence for the total: the certified figure, then the sum of the
    published components, then a rate-based estimate. The breakdown is
    emitted whenever any component exists, independent of the total --
    otherwise a year like 1999, which publishes the three parts but no
    total, silently loses its bar on the chart.
    """
    parts = {k: r.get(f"{k}_levy") for k in ("county", "school", "municipal")}
    has_parts = any(v is not None for v in parts.values())

    if r.get("total_levy"):
        total, source = r["total_levy"], "certified"
    elif has_parts:
        total, source = round(sum(v or 0 for v in parts.values()), 2), "sum_of_parts"
    elif rate:
        total, source = round(ratables * rate / 100), "rate_estimate"
    else:
        total, source = None, None

    return {
        "levy": total,
        "levy_breakdown": parts if has_parts else None,
        "levy_source": source,
    }
...
def town_series(con, rates):
    ...
    for year, ratables, exempt, n, prior_tax in rows:
        ...
        out.append({
            ...
            # Certified levy from DCA when available. Some vintages (1999 among
            # them) publish the school/county/municipal split but leave the
            # total blank, so fall back to summing the parts before falling
            # back again to a rate-based estimate. Gating the breakdown on the
            # total being present would drop a bar whose components exist.
            **_levy_fields(r, ratables, rate),
