...
# A revaluation shows up in the ratio series as a single-year jump upward,
# against the prevailing downward drift of a market that keeps rising while
# the roll stays frozen. Calibrated on Glen Ridge: 1.15 catches the 2008
# revaluation (5.7x) and the 2019 reassessment (1.22x) while excluding 2009,
# whose 1.11x rise was the housing crash pulling prices down to meet the
# assessments rather than any action by the assessor. Detection is a
# heuristic; a flag in rates.csv is authoritative and takes precedence.
REVAL_JUMP = 1.15

# How many recent years of identified sales to carry in the bundle for the
# comparables search. Older sales are poor comps and the bundle has to stay
# small enough to ship as a static file.
COMP_YEARS = 4
...
def build_ratio_study(con, rates=None, residential_only=True):
    ...
    # Revaluation years. rates.csv carries a reval_flag, but it is sparse --
    # only the handful of years with a DCA tax table. A revaluation is also
    # unmistakable in the ratio series itself: the median ratio jumps by a
    # large multiple in a single step as the roll resets to market. Detect
    # those and merge, so the charts mark every reval rather than only the
    # ones that happen to have a rate row.
    medians = {}
    for y in sorted(by_year):
        pairs = by_year[y]
        if len(pairs) >= 10:
            medians[y] = statistics.median(a / p for a, p in pairs)
    detected = set()
    prev_year = prev_med = None
    for y in sorted(medians):
        med = medians[y]
        if prev_med and prev_year == y - 1 and med / prev_med >= REVAL_JUMP:
            detected.add(y)
        prev_year, prev_med = y, med

    def is_reval(y):
        return bool(rates.get(y, {}).get("reval_flag")) or y in detected

    def reval_source(y):
        if rates.get(y, {}).get("reval_flag"):
            return "certified"
        return "detected" if y in detected else None

    for year in sorted(by_year):
        rec = compute(by_year[year])
        rec["year"] = year
        rec["reval_year"] = is_reval(year)
    ...
    for year in all_years:
        ...
        if len(pool) < MIN_SALES:
            continue
        rec = compute(pool)
        # The pool cleared MIN_SALES, but outlier trimming can push it back
        # under. compute() returns a suppressed record in that case; emitting
        # it would put a point with null statistics on every chart and leave
        # the derived findings with a blank where a number should be.
        if rec.get("suppressed"):
            continue
        rec["year"] = year
        rec["window"] = width
        rec["reval_year"] = is_reval(year)
        rec["reval_source"] = reval_source(year)
        rec["state_equalization_ratio"] = rates.get(year, {}).get("equalization_ratio")
    ...
    # Identified recent sales, for the comparables search behind the appeal
    # check. Older sales are useless as comps and would bloat the bundle, so
    # only the last COMP_YEARS are carried, with enough identity to match on
    # location. Owner names are never included (Daniel's Law).
    max_sale_year = max(r[0] for r in rows)
    comps = con.execute(f"""
        SELECT pin, sale_year, sale_price, assessed, prop_class, address
        FROM sales
        WHERE sale_year >= {max_sale_year - COMP_YEARS + 1}
          AND prop_class = '2'
        ORDER BY sale_year DESC, sale_price DESC""").fetchall()
    ...
    return {
        ...
        "comps": [
            {
                "pin": pin,
                "block": pin.split("_")[1] if "_" in pin else None,
                "year": y,
                "price": round(p),
                "assessed": round(a),
                "address": addr,
            }
            for pin, y, p, a, _cls, addr in comps
        ],
        "sales": [
        ...
        "method": {
            ...
            "reval_years_detected": sorted(detected),
