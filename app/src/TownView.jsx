          ...
          <button
            ...
            onClick={() =>
              downloadCSV("glen-ridge-levy.csv", [
                ...
                { label: "total_levy_source", get: (r) => r.levySource },
...
function LevyTooltip({ active, payload, label }) {
  ...
  const source = payload[0]?.payload?.levySource;
  return (
    <div className="levy-tip">
      ...
      {source === "sum_of_parts" && (
        <div className="levy-tip-note">
          total summed from the parts — the state table for this year
          publishes the split but not the total
        </div>
      )}
      {source === "rate_estimate" && (
        <div className="levy-tip-note">
          estimated from ratables x rate — no certified levy published
        </div>
      )}
      {rate && (
